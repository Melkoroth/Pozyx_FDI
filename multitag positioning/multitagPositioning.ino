/**
  Pozyx real-life usage in the FDI of UCM's Madrid campus

  based on The Pozyx ready to localize tutorial (c) Pozyx Labs
...
uint16_t tags[num_tags] = {0x670c, 0x670f, 0x6717, 0x6726, 0x677d};
...
void setAnchorsManual(){
  for (int i = 0; i < num_tags; i++){
    ...
    for(int j = 0; j < num_anchors; j++) {
      ...
      Serial.print(anchor.network_id, HEX);
      Serial.print(",");
      anchor.flag = 0x1;
      anchor.pos.x = anchors_x[j];
      Serial.print(anchor.pos.x);
      Serial.print(",");
      anchor.pos.y = anchors_y[j];
      Serial.print(anchor.pos.y);
      Serial.print(",");
      anchor.pos.z = heights[j];
      Serial.println(anchor.pos.z);
